...
Parents with an **empty Drop** are skipped. Under each drop column: subitems whose **Drop** matches that bucket are listed; subitems with **no** Drop, or whose Drop **does not overlap** the parent’s drops (e.g. typo vs parent), appear under **every** column where the parent appears.
